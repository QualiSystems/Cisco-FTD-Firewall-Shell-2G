...
from fdm_cli_handler import FDMCliHandler
...
from fdm_configuration_flow import FDMConfigurationFlow
from fdm_rest_handler import FDMRestAPIHandler


class CiscoFTDFirewallShell2GDriver(ResourceDriverInterface):
    SHELL_NAME = "Cisco FTD Firewall Shell 2G"
    ...
    def initialize(self, context):
        ...
        FDMCliHandler.set_cli_session_pool_size(resource_config.sessions_concurrency_limit)
    ...
    def restore(self, context, cancellation_context, path, configuration_type, restore_method):
        ...
        resource_config = FirewallResourceConfig.from_context(shell_name=self.SHELL_NAME,
                                                              context=context,
                                                              api=api)
        with LoggingSessionContext(context) as logger:
            api_handler = FDMRestAPIHandler(resource_config=resource_config,
                                            logger=logger)
            restore_flow = FDMConfigurationFlow(logger=logger,
                                                resource_config=resource_config,
                                                api_handler=api_handler)
            restore_flow.restore(path=path,
    ...
    def save(self, context, cancellation_context, folder_path, configuration_type):
        ...
        with LoggingSessionContext(context) as logger:
            api_handler = FDMRestAPIHandler(resource_config=resource_config,
                                            logger=logger)
            save_flow = FDMConfigurationFlow(logger=logger,
                                             resource_config=resource_config,
                                             api_handler=api_handler)
            return save_flow.save(folder_path, configuration_type)
    ...
    def run_custom_command(self, context, cancellation_context, custom_command):
        ...
        api = CloudShellSessionContext(context).get_api()
        resource_config = FirewallResourceConfig.from_context(shell_name=self.SHELL_NAME, context=context, api=api)

        with LoggingSessionContext(context) as logger:
            cli_handler = FDMCliHandler(resource_config, logger)
            RunCommandFlow(logger, cli_handler).run_custom_command(custom_command)
    ...
    def get_inventory(self, context):
        ...
        '''
        resource = CiscoFtdFirewallShell2G.create_from_context(context)
    ...
    def health_check(self, context, cancellation_context):
        ...
        with LoggingSessionContext(context) as logger:
            cli_handler = FDMCliHandler(resource_config, logger)
            state_flow = StateFlow(logger=logger,
                                   resource_config=resource_config,
                                   cli_configurator=cli_handler,
                                   api=api)
            state_flow.health_check()
